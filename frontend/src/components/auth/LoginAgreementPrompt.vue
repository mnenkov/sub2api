<template>
  ...
  >
    <div class="flex items-start gap-3">
      ...
      <div class="min-w-0 flex-1">
        <p class="font-medium">You must accept the latest terms before continuing.</p>
        <p class="mt-1 text-primary-700 dark:text-primary-200/80">
          Password and quick sign-in options remain disabled until you accept.
        </p>
      </div>
      <button
        type="button"
            ...
            <div class="flex items-start gap-4">
              ...
              <div class="min-w-0 flex-1">
                ...
                <p class="mt-2 text-sm leading-6 text-gray-600 dark:text-dark-300">
                  Our service terms were updated on {{ updatedAt || 'a recent date' }}. Please review and accept the following terms before continuing.
                </p>
              </div>
            </div>
          </div>
...
function documentIcon(index: number, title: string): 'document' | 'shield' | 'globe' | 'cog' {
  ...
  if (normalizedTitle.includes('policy') || normalizedTitle.includes('privacy')) {
    return 'shield'
  }
  if (normalizedTitle.includes('countr') || normalizedTitle.includes('region')) {
    return 'globe'
  }
  if (index === 3) {
    return 'cog'
