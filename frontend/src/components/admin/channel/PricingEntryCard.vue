  ...
  <div class="rounded-lg border border-gray-200 bg-gray-50 p-3 dark:border-dark-600 dark:bg-dark-800">
    ...
    >
      ...
      <div v-else class="flex-1 text-xs font-medium text-gray-500 dark:text-gray-400">
        {{ t('admin.channels.form.pricingEntry', 'Pricing entry') }}
      </div>
      ...
      <div class="collapsible-inner">
        ...
        <div class="mt-3 flex items-start gap-2">
          <div class="flex-1">
            <label class="text-xs font-medium text-gray-500 dark:text-gray-400">
              {{ t('admin.channels.form.models', 'Model list') }} <span class="text-red-500">*</span>
            </label>
            <ModelTagInput
              :models="entry.models"
              :platform="props.platform"
              @update:models="onModelsUpdate($event)"
              :placeholder="t('admin.channels.form.modelsPlaceholder', 'Enter a model name and press Enter. Wildcard * is supported.')"
              class="mt-1"
            />
          </div>
          <div class="w-40">
        ...
        <div v-if="entry.billing_mode === 'token'">
          ...
          <label class="mt-3 block text-xs font-medium text-gray-500 dark:text-gray-400">
            {{ t('admin.channels.form.defaultPrices', 'Default prices (used when no interval matches)') }}
          ...
          <div class="mt-1 grid grid-cols-2 gap-2 sm:grid-cols-5">
            ...
            <div>
              <label class="text-xs text-gray-400">{{ t('admin.channels.form.imageTokenPrice', 'Image output') }}</label>
          ...
          <div class="mt-3">
            <div class="flex items-center justify-between">
              <label class="text-xs font-medium text-gray-500 dark:text-gray-400">
                {{ t('admin.channels.form.intervals', 'Context interval pricing (optional)') }}
        ...
        <div v-else-if="entry.billing_mode === 'per_request'">
          ...
          <label class="mt-3 block text-xs font-medium text-gray-500 dark:text-gray-400">
            {{ t('admin.channels.form.defaultPerRequestPrice', 'Default per-request price (used when no tier matches)') }}
          ...
          <div class="mt-3 flex items-center justify-between">
            <label class="text-xs font-medium text-gray-500 dark:text-gray-400">
              {{ t('admin.channels.form.requestTiers', 'Per-request billing tiers') }}
            </label>
            <button type="button" @click="addInterval" class="text-xs text-primary-600 hover:text-primary-700">
          ...
          <div v-else class="mt-2 rounded border border-dashed border-gray-300 p-3 text-center text-xs text-gray-400 dark:border-dark-500">
            {{ t('admin.channels.form.noTiersYet', 'No tiers yet. Add one to configure per-request pricing.') }}
          </div>
        </div>
        ...
        <div v-else-if="entry.billing_mode === 'image'">
          ...
          <label class="mt-3 block text-xs font-medium text-gray-500 dark:text-gray-400">
            {{ t('admin.channels.form.defaultImagePrice', 'Default image price (used when no tier matches)') }}
            <span class="ml-1 font-normal text-gray-400">$</span>
          </label>
          <div class="mt-1 w-48">
            <input :value="entry.per_request_price" @input="emitField('per_request_price', ($event.target as HTMLInputElement).value)"
          ...
          <div class="mt-3 flex items-center justify-between">
            <label class="text-xs font-medium text-gray-500 dark:text-gray-400">
              {{ t('admin.channels.form.imageTiers', 'Image billing tiers (per request)') }}
            </label>
...
const billingModeOptions = computed(() => [
  ...
  { value: 'image', label: t('admin.channels.billingMode.image', 'Image (per request)') }
])
