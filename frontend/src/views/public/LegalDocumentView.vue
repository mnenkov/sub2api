...
const documentIcon = computed<LegalDocumentIcon>(() => {
  const title = (currentDocument.value?.title || '').toLowerCase()
  if (title.includes('policy') || title.includes('privacy')) {
    return 'shield'
  }
  if (title.includes('countr') || title.includes('region')) {
    return 'globe'
  }
