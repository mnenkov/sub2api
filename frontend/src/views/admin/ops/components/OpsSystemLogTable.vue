...
const formatSystemLogDetail = (row: OpsSystemLog) => {
  ...
  // Join with spaces so CSS can wrap at natural boundaries.
  return parts.join('  ')
}
...
const fetchHealth = async () => {
  try {
    ...
    // Health failures should not block the main log flow.
  }
}
...
const saveRuntimeConfig = async () => {
  ...
  try {
    ...
    appStore.showSuccess('Runtime log configuration is now active')
  } catch (err: any) {
...
const cleanupCurrentFilter = async () => {
  const ok = window.confirm('Clean system logs using the current filters? This cannot be undone.')
  if (!ok) return
  try {
    const payload = {
    ...
    appStore.showError(err?.response?.data?.detail || 'Failed to clean system logs')
  }
}
  ...
  <section class="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm dark:border-dark-700 dark:bg-dark-900/60">
    <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
      <div>
        ...
        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Newest logs are shown first. Filter, search, and clean by condition.</p>
      </div>
